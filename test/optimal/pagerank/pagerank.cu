...
init(meta_data mdata, gpu_graph ggraph)
{
	...
	feature_t init_val = 1.0/ggraph.vert_count;
...
{
	std::cout<<"Input: /path/to/exe /path/to/beg_pos /path/to/adj_list /path/weight_list iteration_limit blk_size\n";
...
//	tm_red=wtime()-tm_red;
    ...
    cpu_pagerank<index_t, vertex_t, feature_t>
        (cpu_rank_sum, cpu_dist, iter_limit, ginst->vert_count, ginst->edge_count, ginst->beg_pos,
         ginst->adj_list);
    if ((gpu_rank_sum - cpu_rank_sum)/cpu_rank_sum < 0.00001)
